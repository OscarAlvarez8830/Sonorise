class RemovePaperclipColumns < ActiveRecord::Migration[5.2]
  def change
    remove_column :users, :avatar_file_name
    remove_column :users, :avatar_content_type
    remove_column :users, :avatar_file_size
    remove_column :users, :avatar_updated_at

    remove_column :tracks, :audio_file_name
    remove_column :tracks, :audio_content_type
    remove_column :tracks, :audio_file_size
    remove_column :tracks, :audio_updated_at

    remove_column :playlists, :image_file_name
    remove_column :playlists, :image_content_type
    remove_column :playlists, :image_file_size
    remove_column :playlists, :image_updated_at
  end
end
